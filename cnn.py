...
def try_roc_sklearn(y_pred, y_true):
        ...
        print( "fps, tps, fns, tns = %d, %d, %d, %d",fps,tps,fns, tns)
        ...
        for i in range(n_classes):
                ...
                fnr = 1 - tpr[i]
                err_threshold = thresholdd[np.nanargmin(np.absolute((fnr - fpr[i])))]
...
def try_predict(weights):
        ...
        test_datagen = test_datagen.flow_from_directory(
               '/Users/preethi/Allclass/297/cnn_data/validation/',# '/Users/preethi/Allclass/297/data_validation_unique/',  # this is the target directory
                target_size=(100, 300),  # all images will be resized to 150x150
                batch_size=16,
                shuffle=False,
        ...
        # multiclass
        '''
        y_pred = np.zeros((51, 2), dtype=np.int8)
        ...
        print(y_pred)
        print(y_true_h)
        try_roc_sy_pred = np.zeros((51, 2), dtype=np.int8)
        ...
        y_true = np.zeros((y_true_h.size, int(y_true_h.max()) + 1), dtype=np.int8)
        y_true[np.arange(y_true_h.size), y_true_h] = 1klearn(y_pred, y_true_h)
